...
export const signUp = catch    async (req, res, next) => {
    // try {
    ...
    //     if (error instanceof TypeError && error.message.includes("jwt.sign is not a function")) {
    //         return res.status(500).json({ msg: "JWT signing error" });
    //     }
...
export const verify = async(req , res)=>{
    ...
    jwt.verify(token , privateKey , async(error , decode)=>{
        if(error) return res.json({msg:"somthing went wrong" , error})
