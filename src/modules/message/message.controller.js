...
export const deleteMassage = catchError(async (req, res , next) => {
   
        const { id } = req.params;
        const message = await messageModel.findByIdAndDelete(id)
        if (!message) {
            // return res.json({ msg: "Message is Not exist" })
            next(new AppError("Message is Not exist" , 404))
        }
        return res.json({ msg: "Message is deleted " })
   
})

export const MarkeMassage = catchError(async (req, res , next) => {
